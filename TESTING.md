# Testing Guide for Seating Chart Generator

## Testing Framework

We'll use **Jest** - a popular JavaScript testing framework that works well for both Node.js and browser code.
...
seating-charter/
├── js/
│   ├── seating-generator.js
│   └── app.js
├── tests/
│   ├── seating-generator.test.js
│   └── test-helpers.js
└── package.json
```
...
describe('SeatingChartGenerator', () => {
  ...
  describe('Constraint Checking', () => {
    ...
    test('should prevent students from sitting together at same desk', () => {
      ...
      expect(generator._checkConstraints(seating, 'Bob', 0, 1)).toBe(true);
    });
...
To test the code, you need to export the class. Add this to the end of `seating-generator.js`:

```javascript
// At the end of js/seating-generator.js
...
Aim for:
- **90%+ line coverage**: Most lines of code executed during tests
- **85%+ branch coverage**: Most if/else paths tested
- **100% of public methods**: All public methods have at least one test
...
For testing the full app in a browser, consider:
- **Playwright** or **Cypress** for end-to-end testing
- **Jest + jsdom** for DOM testing without a real browser
